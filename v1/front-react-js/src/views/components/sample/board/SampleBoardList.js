...
const SampleBoardList = () => {
    ...
    const { sampleBoardList, status, statusText } = useSelector((state) => state.sampleBoardReducer);
    ...
    React.useEffect(() => {
        ...
        dispatch(sampleBoardActions.getSampleBoardList(params?.boardType ?? 1));
    ...
    return (
            ...
            {/* <>
        {
          status === 200 ?
            <>
              {sampleBoardList.length > 0 ?
                <div>
                  <div>
                    {
                      sampleBoardList.map((board, index) =>
            ...
            <Link to="/sample/lists/register"><Button variant="text">등록</Button></Link>
                                            ...
                                            {columns.map((column) => {
                                                ...
                                                return (
                                                    <TableCell key={column.id} align={column.align}>
                                                        {
                                                            column.id === 'title' ?
                                                                <Link to={`/sample/lists/${row.id}`}>{value}</Link>
                                                                :
...
export default SampleBoardList;
